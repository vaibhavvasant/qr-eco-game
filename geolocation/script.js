...
const KEY_LOCATIONS = [
  ...
  { id: "waste-quiz", name: "Waste Quiz", lat: 28.423222, lng: 77.06591, radius: 5, type: "waste" },
...
// Check proximity to quiz locations
locationCircles.forEach(({ loc }) => {
  ...
  if (dist <= loc.radius) {
    ...
    // Randomly pick quiz1 or quiz2 for this type
    const quizNumber = Math.random() < 0.5 ? 1 : 2;
    setTimeout(() => {
      window.location.href = `../quizzes/${loc.type}-quiz${quizNumber}.html?type=${loc.type}`;
    }, 500);
  }
});
